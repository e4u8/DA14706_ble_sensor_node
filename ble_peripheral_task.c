                                                            ...
                                                            const uint8_t *value)
{
        ...
        case 0xFF:
                ...
                break;
        default:
                mcs_set_char_value_cfm(svc, conn_idx, ATT_ERROR_APPLICATION_ERROR);
                return;
        }

        /* Update relay GPIO */
        ad_io_configure(relay_gpio_cfg,
                        ARRAY_LENGTH(relay_gpio_cfg),
                        HW_GPIO_POWER_V33,
                        relay_state);

        /* Update characteristic value to reflect current state */
        mcs_char_val = relay_state ? 0x01 : 0x00;

        printf("Relay -> %s\r\n", relay_state ? "ON" : "OFF");

        /* ACK the write request */
...
OS_TASK_FUNCTION(ble_peripheral_task, params)
{
        ...
        ble_gap_device_name_set("BLE_Relay_Ctrl", ATT_PERM_READ);
        ...
        OS_ASSERT(AD_IO_ERROR_NONE == io_err);

        ad_io_configure(relay_gpio_cfg,
                        ARRAY_LENGTH(relay_gpio_cfg),
                        HW_GPIO_POWER_V33,
                        false); /* false = OFF at startup */
